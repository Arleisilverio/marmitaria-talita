...
export default function ClientHome() {
  ...
  const { addItem, updateQuantity, total, items, setStoreSlug } = useCart();
  ...
  if (loading || !menu) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-8 text-white">
      ...
      <p className="font-mono text-xs uppercase tracking-widest text-zinc-500">Carregando Cardápio...</p>
    </div>
  );
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'menu' && (
            <motion.div key="menu-tab" variants={containerVariants} initial="hidden" animate="show">
              ...
              {/* SEÇÃO 1: SE A LOJA TIVER PRODUTOS AVULSOS / ESPETINHOS */}
              {menu.products && menu.products.length > 0 ? (
                <section className="px-container mt-10 space-y-12">
                  {/* Agrupar produtos por categoria */}
                  {Array.from(new Set(menu.products.map((p: any) => p.category || 'Destaques'))).map((categoryName: any) => {
                    const categoryProducts = menu.products.filter((p: any) => (p.category || 'Destaques') === categoryName);
                    return (
                      <div key={categoryName} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {categoryProducts.map((product: any) => {
                            ...
                            const remainingStock = product.stock !== undefined ? product.stock - itemQty : 999;
                            ...
                            return (
                              <motion.div
                                ...
                                className={cn(
                                  "glass-card rounded-3xl p-5 border flex flex-col justify-between transition-all backdrop-blur-xl",
                                  isOutOfStock 
                              ...
                              >
                                <div>
                                  {product.image && (
                                    <div className="h-44 rounded-2xl overflow-hidden mb-4 relative bg-zinc-950">
                                      <img 
                                        src={product.image} 
                                        alt={product.name} 
                                      ...
                                      {product.stock !== undefined && (
                                        <div className="absolute top-3 right-3">
                                          {isOutOfStock ? (
                                            <span className="bg-red-600/90 text-white text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg backdrop-blur-md shadow-lg">
                                              Esgotado
                                            </span>
                                          ) : product.stock <= 5 ? (
                                            <span className="bg-amber-500/90 text-black text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg backdrop-blur-md shadow-lg animate-pulse">
                                              🔥 Restam {product.stock} un!
                                            </span>
                                          ) : (
                                            <span className="bg-black/70 text-zinc-300 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg backdrop-blur-md border border-white/10">
                                  ...
                                  <div className="flex items-start justify-between gap-2 mb-1">
                                    ...
                                    {!product.image && product.stock !== undefined && (
                                      <span className={cn(
                                        "text-[9px] font-mono font-bold uppercase px-2 py-0.5 rounded",
                                        isOutOfStock ? "bg-red-500/10 text-red-500" : "bg-white/5 text-zinc-400"
                                      )}>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-white/5 mt-auto">
                                  <div>
                                    <span className="text-[10px] text-zinc-500 uppercase font-mono block">Preço</span>
                                    <span className="text-white font-black text-lg">{formatBRL(product.price)}</span>
                                  </div>

                                  {isOutOfStock ? (
                                    <span className="px-4 py-2 bg-zinc-800 text-zinc-500 rounded-xl font-bold text-xs uppercase tracking-wider">
                                      Esgotado
                                    </span>
                                  ) : itemQty === 0 ? (
                                    <button
                                      onClick={() => {
                                        if (!menu.isOpen) return toast.error("Loja fechada!");
                                        if (product.stock !== undefined && product.stock <= 0) return toast.error("Produto esgotado!");
                                        addItem({
                                          id: product.id,
                                          name: product.name,
                                          price: product.price,
                                      ...
                                      disabled={!menu.isOpen || isOutOfStock}
                                      className="px-4 py-2.5 bg-primary hover:bg-primary/90 text-white rounded-xl font-bold text-xs uppercase tracking-wider shadow-lg shadow-primary/20 transition-all active:scale-95 disabled:opacity-50"
                                    >
                                      Adicionar
                                    </button>
                                  ) : (
                                    <div className="flex items-center gap-3 bg-zinc-800 border border-white/10 rounded-xl px-2 py-1">
                                      <button
                                        onClick={() => updateQuantity(product.id, undefined, -1)}
                                        className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 text-white font-bold flex items-center justify-center text-sm"
                                      >
                                        -
                                      </button>
                                      <span className="font-bold text-sm text-white px-1">{itemQty}</span>
                                      <button
                                        onClick={() => {
                                          if (product.stock !== undefined && itemQty >= product.stock) {
                                            return toast.error(`Apenas ${product.stock} unidades disponíveis no momento!`);
                                          }
                                          updateQuantity(product.id, undefined, 1);
                                        }}
                                        disabled={remainingStock <= 0}
                                        className="w-7 h-7 rounded-lg bg-primary text-white font-bold flex items-center justify-center text-sm disabled:opacity-40"
                                      >
                                        +
                                      </button>
                                    </div>
                ...
                /* SEÇÃO 2: SE FOR MARMITARIA (MODELO P/M/G) */
                menu.prices && (
                  <section className="px-container mt-12 grid md:grid-cols-2 gap-8">
                    <motion.div variants={itemVariants} className="glass-card rounded-3xl overflow-hidden relative">
                      {menu.image ? (
                        ...
                        <div className="w-full min-h-[300px] bg-zinc-900 flex items-center justify-center text-zinc-700">Sem Foto</div>
                      )}
                    ...
                    <motion.div variants={itemVariants} className="flex flex-col justify-center">
                      ...
                      <p className="text-zinc-400 text-lg mb-8">{menu.description}</p>
                      
                      <div className="mb-8">
                        <p className="text-[10px] text-zinc-500 uppercase font-bold mb-3">Escolha o Tamanho</p>
                        <div className="flex flex-wrap gap-3">
                          {(['p', 'm', 'g'] as const).map(size => (
                            <button 
                              ...
                              className={cn(
                                "py-3 px-6 rounded-2xl font-bold border transition-all", 
                                selectedSize === size 
                                  ? "bg-primary text-white border-primary shadow-lg shadow-primary/20" 
                                  : "bg-zinc-900 text-zinc-400 border-white/5"
                              )}
                            >
                              {size.toUpperCase()} • {formatBRL(menu.prices?.[size] || 0)}
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      <button 
                        ...
                        className={cn(
                          "w-full py-5 rounded-2xl font-heading font-black text-lg shadow-xl transition-all", 
                          menu.isOpen ? "bg-primary text-white hover:scale-[1.02] active:scale-98" : "bg-zinc-800 text-zinc-600"
                        )}
                      >
              ...
              {menu.drinks && menu.drinks.length > 0 && (
                <section className="px-container mt-16 pb-20">
                  <h4 className="font-heading text-2xl font-bold text-white mb-6">Bebidas Geladas & Extras</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {menu.drinks.map((drink: any) => {
                      const cartDrink = items.find(i => i.id === (drink.id || drink.name));
                      const drinkQty = cartDrink?.quantity || 0;

                      return (
                        <div key={drink.id || drink.name} className="glass-card p-4 rounded-2xl flex items-center justify-between border border-white/5 bg-zinc-900/40">
                          <div>
                            <p className="font-bold text-white text-sm">{drink.name}</p>
                            <p className="text-secondary font-bold text-xs">{formatBRL(drink.price)}</p>
                          </div>

                          {drinkQty === 0 ? (
                            <button 
                              onClick={() => handleAddDrink(drink)} 
                              disabled={!menu.isOpen || drink.available === false} 
                              className="w-10 h-10 rounded-xl bg-surface-container text-primary hover:bg-primary hover:text-white flex items-center justify-center transition-colors"
                            >
                              <Plus size={18} />
                            </button>
                          ) : (
                            <div className="flex items-center gap-2 bg-zinc-800 rounded-lg px-2 py-1">
                              <button onClick={() => updateQuantity(drink.id || drink.name, undefined, -1)} className="text-white text-xs font-bold px-1">-</button>
                              <span className="text-white text-xs font-bold">{drinkQty}</span>
                              <button onClick={() => updateQuantity(drink.id || drink.name, undefined, 1)} className="text-primary text-xs font-bold px-1">+</button>
                            </div>
                          )}
                        </div>
                      );
