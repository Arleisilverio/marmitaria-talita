                ...
                todayOrders.map(order => (
                  <div key={order.id} className="bg-zinc-900 border border-white/5 rounded-3xl p-6 flex flex-col shadow-xl">
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <span className={cn("px-3 py-1 rounded-full text-[9px] font-black uppercase", order.status === 'pendente' ? "bg-orange-500/10 text-orange-500" : order.status === 'confirmado' ? "bg-blue-500/10 text-blue-400" : "bg-green-500/10 text-green-500")}>{order.status}</span>
                    </div>
                    ...
                    <div className="bg-black/30 rounded-2xl p-4 mb-6 space-y-2 flex-grow border border-white/5">
                      {order.items_json?.map((item: any, idx: number) => (
                        <div key={idx} className="flex justify-between text-sm">
                          <span className="text-zinc-300"><b className="text-primary mr-2">{item.quantity}x</b> {typeof item.name === 'object' ? item.name.name : item.name}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {order.status === 'pendente' && (
                          <button onClick={() => handleUpdateStatus(order.id, 'confirmado')} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold text-xs uppercase tracking-widest">Confirmar Pedido</button>
                        )}
                        {order.status === 'confirmado' && (
                          <button onClick={() => handleUpdateStatus(order.id, 'entregue')} className="w-full bg-orange-600 text-white py-4 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2">
          ...
          {activeTab === 'menu' && (
            <motion.div key="menu" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
              <div className="flex justify-between items-center">
                ...
                <button onClick={handleSaveMenu} disabled={saving} className="bg-primary px-8 py-3 rounded-xl font-bold text-white shadow-lg shadow-primary/30">
              ...
              <div className="grid lg:grid-cols-2 gap-8">
                <div className="glass-card p-6 rounded-3xl space-y-6">
                  <h3 className="text-zinc-400 text-xs font-black uppercase tracking-widest flex items-center gap-2"><DollarSign className="w-4 h-4 text-primary"/> Preços</h3>
                    ...
                    {['p', 'm', 'g'].map(size => (
                      <div key={size}>
                        ...
                        <div className="relative">
                          ...
                          <input type="text" value={menu.prices[size]} onChange={e => updatePrice(size as any, e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-xl p-3 pl-8 text-white outline-none focus:border-primary"/>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="glass-card p-6 rounded-3xl space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-zinc-400 text-xs font-black uppercase tracking-widest flex items-center gap-2"><Beef className="w-4 h-4 text-primary"/> Carnes</h3>
                    <button onClick={addMeat} className="bg-primary/10 text-primary w-10 h-10 rounded-xl flex items-center justify-center"><Plus/></button>
                  </div>
                    ...
                    {menu.meats?.map((meat: any, idx: number) => {
                      ...
                      return (
                        <span key={idx} className="bg-zinc-800 text-zinc-200 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-3 border border-white/5 group">
                          {meatName}
                          <button onClick={() => setMenu({ ...menu, meats: menu.meats.filter((_:any, i:number) => i !== idx) })} className="text-zinc-600 group-hover:text-red-500"><X className="w-3 h-3"/></button>
                        </span>
                      );
                    })}
                  </div>
                ...
                <div className="glass-card p-6 rounded-3xl space-y-4 lg:col-span-2">
                  <div className="flex justify-between items-center">
                    <h3 className="text-zinc-400 text-xs font-black uppercase tracking-widest flex items-center gap-2"><Coffee className="w-4 h-4 text-primary"/> Bebidas</h3>
                    <button onClick={addDrink} className="bg-primary px-4 py-2 rounded-xl text-[10px] font-black uppercase text-white shadow-lg shadow-primary/20 flex items-center gap-2"><Plus className="w-4 h-4"/> Novo</button>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {menu.drinks?.map((drink: any) => (
                      <div key={drink.id} className="bg-black/40 border border-white/5 p-4 rounded-2xl flex justify-between items-center group">
                        <div>
                          <p className="text-white font-bold text-sm">{drink.name}</p>
                          <p className="text-primary text-xs font-bold">{formatBRL(drink.price)}</p>
                        </div>
                        <button onClick={() => setMenu({ ...menu, drinks: menu.drinks.filter((d: any) => d.id !== drink.id) })} className="text-zinc-700 group-hover:text-red-500"><Trash2 className="w-4 h-4"/></button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {activeTab === 'settings' && (
            <motion.div key="settings" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="max-w-3xl mx-auto space-y-6">
              <div className="glass-card p-8 rounded-3xl space-y-8 border border-white/5 shadow-2xl">
                <h2 className="text-white font-bold text-xl flex items-center gap-2"><Settings className="text-primary"/> Configurações</h2>
