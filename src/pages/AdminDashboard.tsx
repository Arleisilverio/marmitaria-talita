      ...
      <div className="no-print">
        <header className="bg-zinc-950/80 backdrop-blur-xl border-b border-white/10 shadow-[0_0_15px_rgba(255,61,0,0.3)] docked full-width top-0 sticky z-50 flex justify-between items-center px-4 py-3 w-full">
          ...
          <div className="flex items-center gap-4">
            <span className="font-heading text-xs font-bold uppercase tracking-widest text-orange-600 hidden sm:block">ADMIN</span>
          ...
          {activeTab === 'menu' && menu && (
            <div className="space-y-8 pb-12 animate-fade-in">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                 <div><label className="text-zinc-500 text-xs font-mono uppercase mb-2 block">Nome</label><input type="text" value={menu.title} onChange={e => setMenu({...menu, title: e.target.value})} className="w-full bg-zinc-900 border border-white/10 p-4 rounded-xl text-white outline-none" /></div>
                 <div><label className="text-zinc-500 text-xs font-mono uppercase mb-2 block">Descrição</label><textarea value={menu.description} onChange={e => setMenu({...menu, description: e.target.value})} className="w-full bg-zinc-900 border border-white/10 p-4 rounded-xl text-white outline-none resize-none" rows={3} /></div>
              </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                   {['p', 'm', 'g'].map(size => (
                     <div key={size}>
                       ...
                       <input type="number" step="0.01" value={menu.prices[size]} onChange={e => handlePriceChange(size, parseFloat(e.target.value) || 0)} className="w-full bg-zinc-900 border border-white/10 p-4 rounded-xl text-white outline-none text-xl font-bold font-heading" />
                     </div>
                   ))}
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  {menu.meats.map((meat: any) => (
                    <div key={meat.id} className="flex gap-2"><input type="text" value={meat.name} onChange={e => updateMeat(meat.id, 'name', e.target.value)} className="flex-1 bg-zinc-900 p-3 rounded-lg text-white text-sm" /><button onClick={() => updateMeat(meat.id, 'available', !meat.available)} className={`px-3 rounded-lg text-xs font-bold ${meat.available?'bg-green-500/20 text-green-500':'bg-zinc-800 text-zinc-500'}`}>{meat.available?'TEM':'ACABOU'}</button></div>
                  ))}
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between border-b border-white/10 pb-2"><label className="text-zinc-500 text-xs font-mono uppercase">Bebidas e Extras</label><button onClick={addDrink} className="text-orange-500 text-xs font-bold">+ ADC</button></div>
                  {menu.drinks.map((drink: any) => (
                    <div key={drink.id} className="flex gap-2"><input type="text" value={drink.name} onChange={e => updateDrink(drink.id, 'name', e.target.value)} className="flex-1 bg-zinc-900 p-3 rounded-lg text-white text-sm" /><input type="number" value={drink.price} onChange={e => updateDrink(drink.id, 'price', parseFloat(e.target.value))} className="w-20 bg-zinc-900 p-3 rounded-lg text-white text-sm text-center" /></div>
                  ))}
                </div>
              </div>
            </div>
